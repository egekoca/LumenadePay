    ...
    <Screen>
      ...
      <AnimatedContent delay={210}>
        <SurfaceCard padded={false} style={styles.details}>
          ...
          <Detail label={t('Asset')} value={intent.asset.type === 'native' ? 'Native XLM' : `${intent.asset.code} (${intent.asset.type.toUpperCase()})`} />
          {issuer ? <Detail label={t('Issuer')} value={issuer} mono selectable /> : null}
          <Detail label={t('Expires')} value={expiryLabel(remainingLedgers, intent.expiresAtLedger, stellarHealth.isError)} />
